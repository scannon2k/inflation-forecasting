...
```{r fit_models}
# Fit the baseline Phillips curve model using unemployment rate
fitm1 <- train_data %>% 
  model(
    m1 = TSLM(dinfl12 ~ 1 +
...
# Fit the Phillips curve model using 1yr expected inflation
fitm2 <- train_data %>% 
  model(
    m2 = TSLM(dinfl12 ~ 1 +
                 lag(dinfl,12) + lag(dinfl,13) + lag(dinfl,14) +
...
# Fit the Phillips curve model using Michigan inflation expectations
fitm3 <- train_data %>% 
  model(
    m3 = TSLM(dinfl12 ~ 1 +
                 lag(dinfl,12) + lag(dinfl,13) + lag(dinfl,14) +
...
# Fit the Phillips curve model using industrial production
fitm4 <- train_data %>% 
  model(
    m4 = TSLM(dinfl12 ~ 1 +
                 lag(dinfl,12) + lag(dinfl,13) + lag(dinfl,14) +
